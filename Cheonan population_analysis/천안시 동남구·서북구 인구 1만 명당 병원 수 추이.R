...
# 1) 데이터 불러오기 (CSV 파일 경로 지정)
data <- read_csv("C:/Users/김효림/Downloads/2018_2024_인구대비 병원.csv")

# 2) 인구 1만 명당 병원 수 계산
data <- data %>%
  mutate(
    동남구_1만명당_병원수 = (동남구_병원수 / 동남구_인구수) * 10000,
    서북구_1만명당_병원수 = (서북구_병원수 / 서북구_인구수) * 10000
  )
...
# 4) 꺾은선 그래프 그리기
ggplot(data_long, aes(x = 연도, y = 인구1만명당병원수, color = 구, group = 구)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  labs(title = "천안시 동남구·서북구 인구 1만 명당 병원 수 추이",
       x = "연도", y = "인구 1만 명당 병원 수") +
